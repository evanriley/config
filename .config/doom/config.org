...
** Theme
Use auto-dark to switch between light and dark themes.
...
#+end_src
** Modeline
Enable the mu4e modeline alert
#+begin_src emacs-lisp
(setq mu4e-alert-notify-repeated-emails t)
(setq mu4e-alert-enable-notificatoins t)
(after! doom-modeline
...
* Projectile
Set the projectile directory correctly.
#+begin_src emacs-lisp
...
* Org-Mode
** General
Let's unpin org-mode and add org-contrib, org-modern
#+begin_src emacs-lisp :tangle packages.el
(unpin! org)

(package! orb-contrib
  :recipe (:host nil :repo "https://git.sr.ht/~bzg/org-contrib")
  :pin "2380ff9985d3a21aa7746e3e9d1b96a437c748ce")
...
** Super Agenda
Agenda is nice, but let's make it better
#+begin_src emacs-lisp :tangle packages.el
...
~doct~ seems like a nicer way to make setup org-capture
#+begin_src emacs-lisp :tangle packages.el
(package! doct
...
#+begin_src emacs-lisp :noweb-ref none prettify-capture
...
Let's add the ability to capture recipes from common sites and org-ify them.
#+begin_src emacs-lisp :tangle packages.el
(package! org-chef :pin "6a786e77e67a715b3cd4f5128b59d501614928af")
#+end_src

Load it when we try to use it or try to capture a recipe
#+begin_src emacs-lisp
...
;; rename files when moving - needed for mbsync:
  ...
  (setq mu4e-user-mail-address-list '("evan@evanriley.net"))
  ...
  (setq mu4e-contexts
        `(,(make-mu4e-context
            :name "Personal"
            :enter-func
            (lambda () (mu4e-message "Enter evan@evanriley.net context"))
            :leave-func
            (lambda () (mu4e-message "Leave evan@evanriley.net context"))
            :match-func
            (lambda (msg)
              (when msg
                (mu4e-message-contact-field-matches msg
                                                    :to "evan@evanriley.net")))
            :vars '((user-mail-address . "evan@evanriley.net" )
        ...
        (save-excursion
          (let*
              ((from (save-restriction
               ...
               (account
                (cond
                 ((string-match "evan@evanriley.net" from) "migadu"))))
...
(setq mu4e-headers-show-threads nil)
;; copy sent email to the sent folder
...
The current pinned version in doom emacs is not up to date with the pinned version in mu4e.
So we need to unpin evil-collection so that we can use VIm keys in mu4e
#+begin_src emacs-lisp :tangle packages.el
(unpin! evil-collection)
#+end_src

** Prevent emacs using Fish as its shell.
...
issues. To get around this, I will use zsh as my shell in emacs.
